...
def get_navigation(files, config):
    ...
    for i, file in enumerate(missing_from_config):
        ...
        if len(spl) > 2:
            sections.setdefault(spl[0], set()).add(spl[1])
            subsections.setdefault(spl[1], []).append(i)

    for subsection in subsections:
        idxs = subsections[subsection]
        curr_pages = [missing_from_config[i].page for i in idxs]
        _add_previous_and_next_links(curr_pages)
        subsections[subsection] = curr_pages

    in_config = [file.page for file in files.documentation_pages() if file.page and (len(file.page.url.split('/')) == 2)]
    
    for page in in_config:
        section = page.url.split('/')[0]
