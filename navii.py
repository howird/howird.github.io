...
def get_navigation(files, config):
    ...
    for file in files.documentation_pages():
        if file.page is None:
            missing_from_config.append(file)
        elif len(file.src_path.split('/')) == 2:
            in_config.append(file)

    parent_pages = {}
    sections = {}
    for i, file in enumerate(missing_from_config):
        spl = file.src_path.split('/')
        if len(spl) > 2:
            parent_pages.setdefault(spl[0], set()).add(spl[1])
            sections.setdefault(spl[1], []).append(i)

    for section in sections:
        idxs = sections[section]
        curr_pages = []
        for i in idxs:
            curr_pages.append(Page(None, missing_from_config[i], config))
        _add_previous_and_next_links(curr_pages)
        sections[section] = curr_pages
    
    for file in in_config:
        page_name = file.src_path.split('/')[0]
        if page_name in parent_pages:
            file.page.sections = {}
            for section in parent_pages[page_name]:
                curr_pages = sections[section]
                for child in curr_pages:
                    child.parent = file.page
                file.page.sections[section] = curr_pages
